...
ewmh_window_icon_get_unchecked(xcb_window_t w) {
  return xcb_get_property_unchecked(connection, false, w, _NET_WM_ICON,
...
static cairo_surface_t *get_net_wm_icon(xcb_window_t xid,
  ...
  xcb_get_property_cookie_t cookie = xcb_get_property_unchecked(
      connection, false, xid, _NET_WM_ICON, XCB_ATOM_CARDINAL, 0, UINT32_MAX);
...
static xcb_visualtype_t *draw_find_visual(const xcb_screen_t *s,
...
static xcb_visualtype_t *draw_default_visual(const xcb_screen_t *s) {
  return draw_find_visual(s, s->root_visual);
}

static cairo_surface_t *cairo_surface_from_pixmap(xcb_visualtype_t *visual,
                                                  xcb_pixmap_t pixmap,
                                                  xcb_pixmap_t mask) {
  ...
  /* if ((geom_icon_r->depth != 1 && geom_icon_r->depth !=
   * globalconf.screen->root_depth) */
  /*         || (geom_mask_r && geom_mask_r->depth != 1)) */
  /* { */
  /*     warn("Got pixmaps with depth (%d, %d) while processing pixmap, but only
   * depth 1 and %d are allowed", */
  ...
  if (geom_icon_r->depth == 1)
    s_icon = cairo_xcb_surface_create_for_bitmap(
        connection, screen, pixmap, geom_icon_r->width, geom_icon_r->height);
  else
  ...
  if (mask) {
    ...
    result =
        cairo_surface_create_similar(s_icon, CAIRO_CONTENT_COLOR_ALPHA,
                                     geom_icon_r->width, geom_icon_r->height);
    s_mask = cairo_xcb_surface_create_for_bitmap(
        connection, screen, mask, geom_icon_r->width, geom_icon_r->height);
...
static void init_atom(const char *name, xcb_atom_t *dest) {
  xcb_intern_atom_cookie_t c =
      xcb_intern_atom_unchecked(connection, false, strlen(name), name);
  xcb_intern_atom_reply_t *r = xcb_intern_atom_reply(connection, c, NULL);
  *dest = r->atom;
  free(r);
}

static void init_atoms() { INIT_ATOM(_NET_WM_ICON); }

static cairo_surface_t *get_wm_hints_icon(xcb_window_t xid) {
  xcb_get_property_cookie_t wmh_c =
      xcb_icccm_get_wm_hints_unchecked(connection, xid);

  xcb_icccm_wm_hints_t wmh;
  if (!xcb_icccm_get_wm_hints_reply(connection, wmh_c, &wmh, NULL)) {
    return NULL;
  }
  ...
  if (wmh.flags & XCB_ICCCM_WM_HINT_ICON_PIXMAP) {
    if (wmh.flags & XCB_ICCCM_WM_HINT_ICON_MASK)
      result = cairo_surface_from_pixmap(draw_default_visual(screen),
                                         wmh.icon_pixmap, wmh.icon_mask);
  }
  return result;
}

static bool parse_xid(const char *id_str, xcb_window_t *ret) {
  xcb_window_t xid = 0;
